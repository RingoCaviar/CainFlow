...
- 数组输入是批处理信号：`workflow-runner.js` 负责把普通下游节点按数组输入做笛卡尔组合批量执行，确保多图片数组、多文本数组以及图片数组 × 文本数组等组合全部运行到，并把图片结果聚合到 `data.images`、文本结果按输出端口聚合，主文本口同步到 `data.texts`；`execution-core.js` 负责节点 handler、数组输出写入和缓存输出读取。`ImageMerge`、`TextMerge`、`ImagePreview`、`ImageSave`、`Text` 是收集/展示/保存/数组预览节点，接到数组时应一次性接收整组数据，不按数组拆批；`TextChat` 等普通节点接到图片数组或文本数组时应按组合批量运行，避免只处理第一项、只运行一次或只按索引配对漏掉组合。`TextSplit` 这类动态文本输出节点批量执行后，要按真实输出端口（如 `part_1`、`part_2`）分别聚合数组，不要只收集固定 `text` 口。
