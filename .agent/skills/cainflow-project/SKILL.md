...
- 主题样式入口是 `css/themes.css`，它只负责聚合各主题文件；公共主题菜单/按钮样式放 `css/themes/shared.css`；每个主题独立放在 `css/themes/*.css`，当前已有 `dark.css`、`light.css`、`pro.css`。
- 新增或修改任意主题时，默认都要补齐到设置面板、提示词库、历史全屏、缓存抽屉、主题菜单、相机编辑器、Toast、弹窗等细节面板，不要只改主画布和工具栏。
- 处理缓存抽屉时，不能只覆盖 `.cache-info-card`、`.cache-item` 或 `.cache-breakdown` 这些内容块；必须同步检查 `#cache-sidebar > .drawer-content` 这一层的表面色、层级和边界，让抽屉外壳与内部卡片使用同一套主题背景。
- 处理工作流侧栏时，不能只覆盖 `.workflow-item`；必须同步检查 `#workflow-sidebar > .drawer-content`、`.workflow-list-container` 和 `.save-workflow-box`，避免出现抽屉本体、列表项和底部输入区三种背景混在一起的情况。
- 处理工作流侧栏时，不能只覆盖 `.workflow-item`；必须同步检查 `#workflow-sidebar`、`#workflow-sidebar > .drawer-content`、`.workflow-list-container`、`.drawer-footer` 和 `.save-workflow-box`，避免出现抽屉外壳、内容层、列表项和底部保存区分层不一致的情况。
- 处理工作流侧栏时，不能只覆盖 `.workflow-item`；必须同步检查 `#workflow-sidebar.drawer-left`、`#workflow-sidebar > .drawer-content`、`#workflow-sidebar .workflow-item:hover`、`.workflow-list-container`、`.drawer-footer` 和 `.save-workflow-box`，避免出现抽屉外壳、内容层、hover 态和底部保存区分层不一致的情况。
