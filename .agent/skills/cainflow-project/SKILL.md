...
- 图片对比这类交互式媒体节点：端口和默认尺寸放 `js/nodes/types/image-*.js`；节点内容模板放 `js/nodes/node-view-factory.js`；鼠标交互入口放 `js/nodes/node-dom-bindings.js` 并委托到 `js/features/media/media-controller.js`；执行输出与下游刷新放 `js/features/execution/execution-core.js`；节点样式放 `css/components/nodes.css`。
