...
- 发布包是白名单结构：外层 ZIP 只放 `CainFlow.exe`、`LICENSE` / `NOTICE` 和 `workflows/`；`API_DOC`、`.agent`、`.github`、`scripts`、临时 Apifox 文件、构建目录和源码文档不应进入发布包。`js/services/notification-sw.js`、`css`、`js`、`sounds` 等运行时静态资源随 PyInstaller 内置进 exe。
