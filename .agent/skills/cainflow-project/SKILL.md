...
1. 在动手前先给需求分类。
   ...
   - 版本更新检查、更新通知、更新模块显隐、直接下载更新与版本号单一来源 -> `js/core/constants.js`、`js/main.js`、`index.js`、`js/features/update/update-manager.js`、`js/features/settings/settings-controller.js`、`backend/routes/update_routes.py`、`backend/services/update_service.py`、`backend/services/version_service.py`、`backend/config.py`、`change_version.py`
   - 节点定义或序列化 -> `js/nodes/*`
   ...
   - 样式 -> `css/layout/*`、`css/components/*`、`css/features/*`、`css/themes.css`
...
- 代理设置里的“自动检测代理端口”属于设置面板数据逻辑：按钮入口和自动填充/自动保存放 `js/features/settings/settings-controller.js`，后端检测路由放 `backend/routes/settings_routes.py`，常见本地代理端口探测与连通性校验放 `backend/services/security_service.py`。检测成功后再自动勾选代理并填入端口；不要把批量端口探测散到前端。
...
- 并发请求模式开关属于通用设置的“自动化与重试”能力，默认开启；不要只加设置页 UI，还要同步默认状态、配置导入导出、会话恢复和节点序列化中的 `concurrentRequestMode` 字段。
...
- 节点克隆能力的责任链固定为：右键菜单入口与显隐放 `index.html`、`js/features/ui/context-menu-controller.js`；克隆/独立化生命周期与工作流持久化放 `js/nodes/node-lifecycle.js`、`js/nodes/node-serializer.js`；克隆标志、浮动跳转入口与只读 UI 骨架放 `js/nodes/node-view-factory.js`、`css/components/nodes.css`；源节点参数同步、克隆节点编辑锁与跳转到源节点放 `js/nodes/node-dom-bindings.js`；像 `ImageImport` 这类带媒体导入模式的参数同步要同时检查 `js/features/media/media-controller.js` 与 `index.js` 的注入链。克隆节点应保持“参数跟随源节点、输入输出端口可正常使用、节点自身位置/折叠状态独立、运行结果不与源节点共用”的边界；只有执行“独立化克隆节点”后，才变回普通可编辑节点。
...
- 备用输入口（Spare Input Port）指动态输入端口始终保持“已连接端口最大序号 + 1”，保证末尾总有 1 个空输入口可继续连接。`ImageMerge` / `TextMerge` 使用该规则：节点定义放 `js/nodes/types/image-merge.js` / `js/nodes/types/text-merge.js`，端口重建与失效连线清理放 `js/nodes/node-dom-bindings.js`，初始端口数量由 `js/nodes/node-view-factory.js` 读取 `inputCount`，保存/复制同步 `js/nodes/node-serializer.js` 与 `js/features/ui/clipboard-controller.js`。
...
- `Text` 节点有 1 个文本输入口和 1 个文本输出口。执行时在 `js/features/execution/execution-core.js` 负责把上游输入写入文本框、同步 `node.data.text` / `node.data.texts` 并刷新连线输出；收到多文本数组时用左右箭头切换显示，`textPreviewIndex` 记录当前项，textarea 编辑只更新当前文本项，输出口继续返回整组文本数组。不要在运行 handler 中调用 `requestNodeFit` / `fitNodeToContent`，运行后不应自动改变节点尺寸。
- `TextSplit` 节点有 1 个文本输入口，不再提供节点内部长文本输入框；输入文本来自上游连接或旧工作流恢复数据。输出口数量由节点内“输出数量”步进控件控制：大于 0 时按配置数量生成 `part_N` 端口，等于 0 时根据分割结果自动生成 `part_N` 端口，并在控件底部提示用户；输入框只允许数字，左右按钮负责增减。开启“多合一输出”后，输出端口收敛为单个 `text` 端口，输出内容是所有分割片段组成的多文本数组，`getCachedOutputValue` 应优先从 `node.data.texts` / `node.data.parts` 读取该数组。端口模板、输出数量控件、多合一输出开关、节点内预览与动态重建放 `js/nodes/node-view-factory.js`、`js/nodes/node-dom-bindings.js`，执行输出分发与动态端口取值放 `js/features/execution/execution-core.js`，保存/复制时同步 `delimiter`、`outputCount`、`removeEmptyLines`、`previewEnabled`、`mergeOutputEnabled`、`parts` 等字段到 `js/nodes/node-serializer.js` 与 `js/features/ui/clipboard-controller.js`，恢复链路放 `js/nodes/node-lifecycle.js`。旧工作流没有 `outputCount` 时，应继续从已有 `parts` 或分割结果推导端口数量，避免旧连线失效。开启节点内预览后，预览区应在节点内部 flex/滚动展示，不得把完整预览内容计入节点最小高度或阻止用户手动缩小节点。
...
- `TextChat` 现在也带节点底部“生成进度”框，沿用 `ImageGenerate` 的进度框样式。静态结构继续放 `js/nodes/node-view-factory.js` / `css/legacy.css`，运行态进度数字更新放 `js/features/execution/execution-core.js`，批量输入下的总调用次数预计算放 `js/features/execution/workflow-runner.js`；新增或修改这块时不要只改其中一段链路。
...
- `ImagePreview` / `ImageSave` 都支持多图输入，运行态图片数组使用 `node.data.images`、`node.imageDataList` 或 `node.data.generatedImages` 表示，当前索引用 `imagePreviewIndex` 类字段记录；节点内用左右箭头和计数器切换当前图。多图结果的累积、`getCachedOutputValue` 多图输出读取，以及对展示/保存节点的专门分发放 `js/features/execution/execution-core.js` / `js/features/execution/workflow-runner.js`，节点内多图预览切换、当前图全屏、手动/自动批量保存与编号文件名放 `js/features/media/media-controller.js`；节点静态结构仍由 `js/nodes/node-view-factory.js` 输出，样式收口在 `css/legacy.css`。
- `ImageMerge`（多图合一）把多个 `image_N` 输入按端口顺序展平成一个图片数组，从单个 `image` 输出口输出；`TextMerge`（多文本合一）把多个 `text_N` 输入按端口顺序展平成一个文本数组，从单个 `text` 输出口输出。两类合一节点用于收集数组，不应被批量执行拆分。
...
- 数组输入是批处理信号：`workflow-runner.js` 负责把普通下游节点按数组输入做笛卡尔组合批量执行，确保多图片数组、多文本数组以及图片数组 × 文本数组等组合全部运行到，并把图片结果聚合到 `data.images`、文本结果按输出端口聚合，主文本口同步到 `data.texts`；`execution-core.js` 负责节点 handler、数组输出写入和缓存输出读取。`ImageMerge`、`TextMerge`、`ImagePreview`、`ImageSave`、`Text` 是收集/展示/保存/数组预览节点，接到数组时应一次性接收整组数据，不按数组拆批；`TextChat` 等普通节点接到图片数组或文本数组时应按组合批量运行，避免只处理第一项、只运行一次或只按索引配对漏掉组合。`TextSplit` 这类动态文本输出节点批量执行后，普通模式要按真实输出端口（如 `part_1`、`part_2`）分别聚合数组；开启 `mergeOutputEnabled` 时则单个 `text` 口代表多合一文本数组，应把所有批次的片段继续聚合到 `data.texts`。
- API 调用类节点的进度总数要在真正执行前先算出来：这条预计算链路放 `js/features/execution/workflow-runner.js`，运行态逐次递增放 `js/features/execution/execution-core.js`。`TextChat` 的总数等于本轮批量输入展开后的运行次数，`ImageGenerate` 的总数等于“批量输入展开后的运行次数 × generationCount”；失败不计入进度，自动重试时沿用运行时进度状态继续补剩余次数。
- 并发请求模式是通用设置中的全局开关，状态名为 `concurrentRequestMode`，默认开启。新增或修改该开关时要同步 `js/core/state.js`、`js/features/settings/settings-controller.js`、`js/features/ui/ui-controller.js`、`js/features/persistence/project-io.js`、`js/nodes/node-serializer.js` 与 `index.js` 的注入链，保证刷新、导入导出、会话恢复和节点序列化一致。
- 并发请求模式打开后，`workflow-runner.js` 负责识别同一 `ImageGenerate` / `TextChat` 节点因数组输入即将执行多次的场景，并用 `executeConcurrentBatchRequest` 并发发起每组请求；所有请求独立返回，失败项按自动重试次数单独重试，只有全部成功后才通过 `commitConcurrentBatchResults` 一次性打包结果并继续下游。
- `ImageGenerate` 在并发请求模式下还会把单节点 `generationCount > 1` 的多次生图改为内部并发请求。并发分支只用局部数组收集结果，普通运行失败时通过 `commitImageGenerateOutputs` 保留已成功图片和 `generationCompletedCount`，自动重试只补剩余次数；批量并发运行时不要提前写入共享 `node.data.image` / `node.data.images`，必须等全部 batch 成功后统一提交，避免多个并发 batch 污染同一个节点对象。
- `TextChat` 勾选“固定结果”且已有缓存结果时，应被视为独立缓存源：执行计划收集上游时要停止继续追溯它的输入，`getCachedOutputValue` 只输出 `node.data.text` 单条缓存，`shouldRunNodeForEachInput` 不得因上游数组把它批量展开，handler 命中缓存时也不能再检查模型、供应商或 API Key，更不能发起请求。若上次结果来自批量对话，固定结果模式下也只向下游输出最后缓存的单条 `data.text`，不要继续透出旧 `data.texts` 数组。
- 并发请求模式下的节点上方点阵状态 UI 只属于运行态反馈：`js/features/execution/workflow-runner.js` 负责创建/重置 `.node-concurrent-status-panel`、计算请求总数并把每个请求映射到一个点；`js/features/execution/execution-core.js` 在 `ImageGenerate` 内部并发生成请求完成或失败时回传单点状态；`css/components/nodes.css` 负责悬浮在节点上方的圆角面板、圆点网格与黄色/绿色/红色状态。该 UI 不写入 `node.data`、不进入 `node-serializer.js`、不进入缓存或工作流导出，页面刷新后自然消失；尺寸使用节点坐标系，宽度等于当前卡片宽度，高度按点数量自动换行增高，不要用 `--canvas-zoom` 做反向缩放。
...
- 右键菜单添加节点入口使用“一级分类 + 二级节点列表”：分类入口结构放 `index.html`，用 `data-submenu-target` 指向对应二级菜单；二级菜单用 `data-context-submenu` 标记，并在具体节点项上保留 `data-type`。打开/关闭、视口避让、悬停延迟和点击分发放 `js/features/ui/context-menu-controller.js`；菜单视觉、箭头和打开态放 `css/legacy.css`，浅色主题覆盖放 `css/themes.css`。新增静态节点时优先放入合适分类，只改菜单结构和 `data-type`，不要把节点创建逻辑写进 `index.html`。
...
- 缓存侧栏里的“占用空间”统计与清理边界要和 IndexedDB 分层保持一致：`js/features/settings/settings-controller.js` 负责把“历史记录”统计为 `STORE_HISTORY` 元数据 + `STORE_ASSETS` 中 `history:` 前缀原图，把“节点资产”统计为 `STORE_ASSETS` 中非 `history:` 资产，把“基础数据”统计为 `localStorage`；`js/features/ui/ui-controller.js` 的缓存面板按钮只能做分项清理，不得顺手清掉 API 供应商、模型配置或其他 `localStorage` 设置。
...
- 克隆节点的“同步”默认只指参数与配置，不应把源节点的运行结果、完成态或错误态直接镜像给克隆节点；否则会破坏克隆节点作为独立输入输出节点的执行语义。修改克隆同步逻辑时，优先检查 `js/nodes/node-dom-bindings.js`、`js/features/execution/execution-core.js`、`js/features/execution/workflow-runner.js` 的边界是否仍成立。
...
- 代理设置是“前端状态 + 后端全局代理状态”的双端能力：前端设置页保存代理时立即 `POST /api/proxy`，应用启动时 `js/features/app/startup-controller.js` 在 `loadState()` 结束后无论是否恢复出节点，都要调用 `syncProxyToServer()` 把本地代理配置恢复到后端，避免空画布启动后更新下载拿不到代理。
...
- `/proxy` 的异常归类边界也固定在 `backend/services/proxy_service.py`：`BrokenPipeError` / `ConnectionResetError` 这类浏览器真正断开可记为 `client_disconnect`，但 `http.client.RemoteDisconnected` 属于上游连接在响应前关闭，必须继续按上游断连处理并返回可读错误，不能误记成客户端断开后让前端只看到 `Failed to fetch`。
...
- `ImageGenerate` 的节点内进度框不再只反映单次 `generationCount` 循环；如果节点因为图片数组、文本数组或两者组合而被批量执行，底部进度应显示“本轮总 API 调用次数”的完成度。总数预计算在 `js/features/execution/workflow-runner.js`，单次成功后的递增与失败回退文案在 `js/features/execution/execution-core.js`。
...
- 输入端口改线交互的责任链固定为：端口 `mousedown` 识别已连接输入口、摘除旧 incoming connection、建立从原输出端继续拖拽的 `state.connecting` 标记放 `js/nodes/node-dom-bindings.js`；拖拽移动、松手和“改线拖到空白处只断开、不弹创建节点候选”的判定放 `js/canvas/canvas-interactions.js`；最终连线提交、替换目标输入口和撤销历史复用放 `js/canvas/connections.js`。不要把已连接输入口拖拽当作普通输出端拖线，否则松到空白处会误弹可创建节点菜单。
- 从端口拖到空白处松手后弹出的“可创建并自动连接”的节点候选，也属于 `js/canvas/connections.js` 的职责。像 `ImageGenerate` 这类有多个同类型输入口的节点，不要只按“第一个兼容端口”硬连：文本输出创建 `ImageGenerate` 时，只有 `CameraControl` 的文本输出默认连接到 `camera_prompt`，其他文本节点都应默认连接到 `prompt`，避免把普通提示词误接到视角提示词输入。
...
- 版本号的单一来源现在是 `js/core/constants.js` 的 `APP_VERSION_NUMBER`：前端展示、静态资源缓存参数与 `APP_ASSET_VERSION` 由它派生，后端展示与 User-Agent 通过 `backend/services/version_service.py` 运行时读取同一值，版本同步脚本入口在 `change_version.py`。升级版本时不要再恢复 `package.json` 或 `css/base/variables.css` 这类第二份版本号；如果当前会话显式要求更新 skill/架构图，也要同步更新 `.agent/skills/cainflow-project/SKILL.md` 和 `references/architecture-map.md` 里的版本信息与相关经验规则。
...
- 修改并发请求模式、API 调用类节点批量并发、失败项自动重试或并发结果打包语义
- 修改 TextChat “固定结果”缓存、上游依赖追溯、数组输入批量展开或缓存输出语义
